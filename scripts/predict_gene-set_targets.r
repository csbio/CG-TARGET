...
option_list = list(
  make_option(
    ...
    action = 'store_true',
    default = FALSE,
    help = 'Performs predictions for a small subset of the data, to ensure that everything is working properly before the full set of predictions are made (can take days for large screens). This option only has an effect when "--load_point" is not specified or is 0. If "--test_run" has been specified and the analysis saved halfway through using "--save_points," then the analysis will still be in "test run" mode when loading that save point, and vice versa if "--test_run" was not specified. NOTE: the results from this run will not be accurate!!!'
  ),
  make_option(
    '--load_point',
    default = 0,
    type = 'integer',
    help = 'Point in the processing at which a previously partially-processed, saved dataset should be loaded and its processing continued through the rest of the script. Possible values are in c(0, 1, 2, 3, 4, 5, 6). 0 (default): run all code from the beginning; 1: resume the analysis at per-gene-set p-value computations; 2: resume the analysis at per-condition p-value computations; 3: Combine the p-values and z-scores so that there is one p-value and z-score per condition X gene set pair; 4: Compute and assemble the drivers of the gene-set predictions; 5: Table mashing; 6: Save after End of Analysis - useful for debugging. While primarily for testing purposes, this could be useful during real analyses.'
  ),
  make_option(
    '--save_points',
    default = '',
    type = 'character',
    help = 'Point(s) in the processing at which to save the data in their present, partially-processed form. See "--load_point" for descriptions of these points. If the user wishes to specify multiple save points, these must be comma-delimited *without* spaces (e.g. --save_points 1,2). Data are saved in "*.RData" format.'
  )
)
...
if (load_point < 1) {
  ...
  gene_prediction_tab = fread(
    sprintf('gzip -dc %s', gene_pred_file),
    header = TRUE,
  ...
  zero_resampled_profiles = (config_params$Required_arguments$`per-array_resampling_scheme` == 0) |
  ...
  if (!zero_resampled_profiles) {
    ...
    rand_gene_prediction_tab = fread(
      sprintf('gzip -dc %s', rand_gene_pred_file),
      header = TRUE,
      colClasses = c('character', 'character', 'character', 'numeric')
    )
  } else {
  ...
  sample_table = fread(sample_table_filename,
                       header = TRUE,
  ...
  print(gene_set_info)
  gene_set_tab_full = fread(
    gene_set_info$filename,
    header = TRUE,
    sep = '\t',
    colClasses = 'character'
  )
  ...
  if (is.na(min_term_size) | is.na(max_term_size)) {
    stop(
      'Required arguments "min_term_size" and/or "max_term_size" not specified\nproperly. Please specify as a number and re-run this script.'
    )
  }

  gi_info = get_gi_info(config_params$Required_arguments$gi_dataset_name,
                        TARGET_PATH)
  ...
  if (is.na(driver_cutoff)) {
    stop('driver_cutoff argument must be numeric.')
  }
  ...
  if (!is.null(gene_name_column)) {
    if (!gene_name_column %in% names(query_info_tab)) {
      stop(
        sprintf(
          'column %s was specified as the column in the query info table that contains interpretable gene names, but the columns does not exist! query info table is here:\n%s',
          gene_name_column,
          gi_info$gi_query_tab
        )
      )
    }
  }
  ...
  if (!is.null(dummy_name)) {
    ...
    dummy_dt = fread(
      sprintf(
        'gzip -dc %s',
        file.path(
          get_dummy_folder(dummy_name),
          dummy_config_params$cg_data_table
        )
      ),
      colClasses = c(
        'character',
    ...
    dummy_col_tab = fread(
      file.path(
        get_dummy_folder(dummy_name),
        dummy_config_params$cg_col_info_tab
      ),
      header = TRUE,
      colClasses = 'character'
    )
    ...
    if (!is.null(config_params$Options$dummy_dataset$negative_control_column)) {
      ...
      message(
        sprintf(
          'dummy matrix dimensions before filtering for "cols_to_include": (%s, %s)',
    ...
    sample_table = add_dummy_controls_to_cg_sample_tab(
      sample_table,
  ...
  if (!is.null(cond_name_col)) {
    ...
    condition_name_tab = sample_table[, list(
      condition = sprintf('%s_%s', screen_name, expt_id),
      condition_name = sample_table[[cond_name_col]]
    )]
    ...
    # predictions!**
    if (!zero_resampled_profiles) {
      rand_condition_name_tab = unique(rand_gene_prediction_tab[, list(screen_name, expt_id)])[, list(
        condition = sprintf('%s_%s', screen_name, expt_id),
        condition_name = sprintf('%s_%s', screen_name, expt_id)
      )]
  ...
  gene_prediction_mat = acast(gene_prediction_tab,
                              screen_name + expt_id ~ query_key,
                              value.var = 'score')
  if (!zero_resampled_profiles) {
    rand_gene_prediction_mat = acast(rand_gene_prediction_tab,
                                     screen_name + expt_id ~ query_key,
                                     value.var = 'score')
...
if (load_point < 2) {
  ...
  if (!zero_resampled_profiles) {
    ...
    if (length(overlap_conds) > 0) {
      stop(
        sprintf(
          'The following real and random conditions have the same name:\n%s',
          paste(overlap_conds, collapse = '\n')
        )
      )
    }
  }
  ...
  print(sprintf(
    'Number of %s conditions: %s',
    names(table(all_control_vec)),
    table(all_control_vec)
  ))
  ...
  if (!is.null(gene_name_column)) {
    ...
    best_query_name_mat = matrix(
      gene_name_map[best_query_mat],
      nrow = nrow(best_query_mat),
      ncol = ncol(best_query_mat),
      dimnames = dimnames(best_query_mat)
    )
  } else {
    best_query_name_mat = matrix(
      NA_character_,
  ...
  controls <-
    intersect(sample_types_to_predict_for,
              c('expt_control', 'rand-by-strain'))
  ...
  gene_set_matrix = acast(
    data = gene_set_tab,
    formula = gene_set_mat_formula,
  ...
  best_prediction_mat <-
    best_prediction_mat[, common_genes, drop = FALSE]
  ...
  print(sprintf(
    'Removed %s gene sets with no annotations',
    sum(!nonzero_degree_gene_set_inds)
  ))
  ...
  gene_set_matrix = gene_set_matrix[, term_size_min_pass &
                                      term_size_max_pass, drop = FALSE]
  print(sprintf(
    'Removed %s gene sets with fewer than %s annotations',
    sum(!term_size_min_pass),
    min_term_size
  ))
  print(sprintf(
    'Removed %s gene sets with greater than %s annotations',
    sum(!term_size_max_pass),
    max_term_size
  ))
  ...
  setnames(accepted_annotation_tab,
           c(gene_set_gene_id_col, gene_set_id_col))
  gene_set_tab_final = copy(gene_set_tab_full)
  setkeyv(gene_set_tab_final,
          c(gene_set_gene_id_col, gene_set_id_col))
  ...
  test_inds <-
    do.call(c, lapply(unique(all_control_vec), function(x) {
  ...
  condition_gene_score_stdevs <-
    apply(target_prediction_mat, 1, sd)
  ...
  condition_subset_gene_set_pred_sum_mat <-
    condition_gene_set_pred_sum_mat[sample_type_split_vec %in% sample_types_to_predict_for,]
  target_prediction_subset_mat <-
    target_prediction_mat[sample_type_split_vec %in% sample_types_to_predict_for,]
...
if (load_point < 3) {
  if (!(is.null(neg_control_col) & zero_resampled_profiles)) {
    ...
    per_gene_set_pvals_zscores = compute_per_gene_set_pvals_zscores(
      condition_subset_gene_set_pred_sum_mat,
      control_gene_set_pred_sum_mat_list,
      control_gene_set_pred_sum_means_list,
      control_gene_set_pred_sum_stdevs_list,
      controls
    )
  } else {
...
if (load_point < 4) {
  ...
  per_condition_pvals_zscores = compute_per_condition_pvals_zscores_2(
    target_prediction_subset_mat,
    condition_subset_gene_set_pred_sum_mat,
    gene_set_matrix,
    num_per_condition_rand,
    gene_set_sizes,
    condition_subset_gene_score_means,
    condition_subset_gene_score_stdevs,
    seed
  )
...
if (load_point < 5) {
  ...
  if (length(all_pvals_zscores[['per_gene_set']]) == 2) {
    per_gene_set_worst_case_mats <-
      get_worst_case_pval_zscore(
        all_pvals_zscores[['per_gene_set']][[controls[1]]][['pval']],
    ...
    print(str(all_pvals_zscores))
    per_gene_set_worst_case_mats = list(
      worst_pval = all_pvals_zscores[['per_gene_set']][[controls[1]]][['pval']],
      worst_zscore = all_pvals_zscores[['per_gene_set']][[controls[1]]][['zscore']],
      control_name = matrix(
        controls[1],
        nrow = dim(all_pvals_zscores[['per_gene_set']][[controls[1]]][['pval']])[1],
        ncol = dim(all_pvals_zscores[['per_gene_set']][[controls[1]]][['pval']])[2],
        dimnames = dimnames(all_pvals_zscores[['per_gene_set']][[controls[1]]])
      )
    )
    ...
    stop(
      sprintf(
        'Not sure how this happened, as there are %s control types when there should be either 1 or 2',
        length(controls)
      )
    )
  }
  ...
  if (length(all_pvals_zscores[['per_gene_set']]) == 0) {
    ...
    overall_worst_case_mats = list(
      worst_pval = all_pvals_zscores[['per_condition']][['pval']],
      worst_zscore = all_pvals_zscores[['per_condition']][['zscore']],
      control_name = matrix(
        'per_condition',
        nrow = dim(all_pvals_zscores[['per_condition']][['pval']])[1],
        ncol = dim(all_pvals_zscores[['per_condition']][['pval']])[2],
        dimnames = dimnames(all_pvals_zscores[['per_condition']][['pval']])
      )
    )
    ...
    overall_worst_case_mats = list(
      worst_pval = per_gene_set_worst_case_mats[['worst_pval']],
      worst_zscore = per_gene_set_worst_case_mats[['worst_zscore']],
      control_name = matrix(
        'per_gene_set',
        nrow = dim(all_pvals_zscores[['per_gene_set']][['pval']])[1],
        ncol = dim(all_pvals_zscores[['per_gene_set']][['pval']])[2],
        dimnames = dimnames(all_pvals_zscores[['per_gene_set']][['pval']])
      )
    )
  } else {
    # Otherwise, there is at least one per-condition and one per-gene-set
    # set of statistics, this is how to combine them.
    overall_worst_case_mats <-
      get_worst_case_pval_zscore(
        per_gene_set_worst_case_mats$worst_pval,
  ...
  if (exists('per_gene_set_worst_case_mats')) {
    per_gene_set_indices <-
      overall_worst_case_mats$control_name == 'per_gene_set'
    overall_worst_case_mats$control_name[per_gene_set_indices] <-
      per_gene_set_worst_case_mats$control_name[per_gene_set_indices]
  }
}
...
if (load_point < 6) {
  ...
  gene_set_drivers_dt = get_gene_set_drivers(
    best_prediction_subset_mat,
    best_query_subset_mat,
    best_query_name_subset_mat,
    gene_set_matrix,
...
if (load_point < 7) {
  ...
  overall_worst_case_df <-
    lapply(overall_worst_case_mats, as.vector)
  overall_worst_case_df <- as.data.frame(overall_worst_case_df)

  overall_GENE_SET <-
    as.character(col(overall_worst_case_mats$worst_pval, as.factor = TRUE))
  overall_CONDITION <-
    as.character(row(overall_worst_case_mats$worst_pval, as.factor = TRUE))

  overall_worst_case_dt <-
    data.table(condition = overall_CONDITION,
               gene_set = overall_GENE_SET,
               overall_worst_case_df)
  ...
  overall_worst_case_dt <-
    gene_set_drivers_dt[overall_worst_case_dt]
  ...
  overall_worst_case_dt <-
    overall_worst_case_dt[order(worst_pval, -worst_zscore)]
  ...
  setnames(overall_worst_case_dt,
           c('worst_pval', 'worst_zscore'),
           c('p_value', 'z_score'))
  if (is.null(gene_set_name_tab))
    cols = c(cols[1:(which(cols == 'gene_set_name') - 1)], cols[(which(cols == 'gene_set_name') + 1):length(cols)])
  if (is.null(condition_name_tab))
    cols = c(cols[1:(which(cols == 'condition_name') - 1)], cols[(which(cols == 'condition_name') + 1):length(cols)])
  if (is.null(gene_name_column))
    cols = c(cols[1:(which(cols == 'driver_name') - 1)], cols[(which(cols == 'driver_name') + 1):length(cols)])
